...
- 等价分支合并：仅 DER 不同、但主体/公钥/上级、有效期、层级与约束、EKU、
  策略内容和撤销结论全部相同的交叉签发证书，在**所有裁决门**上等价；
  路径枚举按等价类（互模拟分区）进行，门只在等价类路径上求值一次。
  这不放宽任何规则，只消除对同一结果的指数级重复计算；决胜仍解析回
  字典序最小的具体指纹序列。
- 无有效路径时返回**覆盖所有可达候选分支**的拒绝证明：
  ...
  **首个失败规则**（`SIGNATURE`/`VALIDITY`/`BASIC_CONSTRAINTS`/`KEY_USAGE`/
  ...
  `NO_PATH_TO_ANCHOR` …），而不是只返回最后尝试的一条链。等价分支合并时
  以 `path_level_failures[].path_count` 给出该 `(锚, 规则, 失败位置)`
  覆盖的具体候选路径数，并附字典序最小的 `example_path`。
